...
# Define PolynomialFeatures to transform features
poly = PolynomialFeatures(degree=3, include_bias=False) # Determine the degree of the polynomial
X_poly = poly.fit_transform(X) # Convert features
...
for train_index, test_index in kf.split(X, y):
    ...
    perceptron_predictions = perceptron_model.predict(X_test_poly) 

    # LDA Model
    lda_model = LinearDiscriminantAnalysis()
    lda_model.fit(X_train_poly, y_train) 
    lda_predictions = lda_model.predict(X_test_poly) 
    ...
    # Display decision regions (for Perceptron and LDA)
    x_min, x_max = X[:, 0].min() - 1, X[:, 0].max() + 1
    y_min, y_max = X[:, 1].min() - 1, X[:, 1].max() + 1
    xx, yy = np.meshgrid(np.arange(x_min, x_max, 0.02), np.arange(y_min, y_max, 0.02))
    Z_perceptron = perceptron_model.predict(poly.transform(np.c_[xx.ravel(), yy.ravel()])) 
    ...
    Z_lda = lda_model.predict(poly.transform(np.c_[xx.ravel(), yy.ravel()])) 
